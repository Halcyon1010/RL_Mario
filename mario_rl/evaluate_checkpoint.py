...
def is_better_episode(candidate, current, key):
    if current is None:
        return True
    if key == "flag_get":
        candidate_score = (candidate["flag_get"], candidate["max_x_pos"], candidate["total_reward"])
        current_score = (current["flag_get"], current["max_x_pos"], current["total_reward"])
...
def main():
    ...
    parser.add_argument("--save-best-by", default="max_x_pos", choices=["max_x_pos", "total_reward", "flag_get"])
    parser.add_argument("--stop-on-flag", action="store_true", help="Stop rollout search as soon as a flag-reaching episode is found.")
    parser.add_argument(
        "--terminal-hold-frames",
        type=int,
        default=0,
        help="Append copies of the final frame so terminal states are visible in the video.",
    )
    ...
    for episode_idx in range(args.episodes):
        ...
        if not args.no_video and frames and args.terminal_hold_frames > 0:
            frames.extend([frames[-1].copy() for _ in range(args.terminal_hold_frames)])
        ...
        if is_better_episode(summary, best, args.save_best_by):
            best = summary
        if args.stop_on_flag and flag_get:
            break
