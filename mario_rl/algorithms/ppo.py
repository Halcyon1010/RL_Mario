...
def summarize_eval(results):
    ...
    return {
        ...
        "max_max_x_pos": float(max(row["max_x_pos"] for row in results)),
        "flag_rate": float(np.mean([row["flag_get"] for row in results])),
        "any_flag_get": int(any(row["flag_get"] for row in results)),
...
def evaluate(agent, config, device, global_step, run_path, logger, split, env_ids, episodes, max_episode_steps, eval_mode, wandb_run=None):
        ...
        for episode_idx in range(int(episodes)):
            ...
            wandb_log(
                wandb_run,
                {
                    ...
                    f"{split}/{eval_mode}/flag_get": int(flag_get),
                },
                step=global_step,
            )
            results.append(row)
    ...
    wandb_log(
        wandb_run,
        {
            ...
            f"{split}/max_max_x_pos": summary["max_max_x_pos"],
            f"{split}/flag_rate": summary["flag_rate"],
            f"{split}/any_flag_get": summary["any_flag_get"],
            ...
            f"{split}/{eval_mode}/max_max_x_pos": summary["max_max_x_pos"],
            f"{split}/{eval_mode}/flag_rate": summary["flag_rate"],
            f"{split}/{eval_mode}/any_flag_get": summary["any_flag_get"],
...
def train(config_path, overrides=None):
    ...
    best_val_mean_max_x = -float("inf")
    best_val_single_max_x = -float("inf")
    best_summary = None
    best_max_x_summary = None
    best_single_max_x_summary = None
    ...
    while global_step < total_timesteps:
        ...
        for step in range(rollout_steps):
            ...
            episode_flag_get = episode_flag_get or bool(info.get("flag_get", False))
            ...
            if done:
                ...
                train_logger.write(
                    train_row := {
                        ...
                        "episode_flag_get": int(episode_flag_get),
                ...
                wandb_log(
                    wandb_run,
                    {
                        ...
                        "train/episode_flag_get": int(episode_flag_get),
            ...
            if global_step >= next_eval_step:
                ...
                for eval_mode in eval_modes:
                    mode_results, mode_summary = evaluate(
                        agent,
                        config,
                        device,
                        global_step,
                    ...
                    val_episode_rows[eval_mode] = mode_results
                val_summary = val_summaries[best_model_eval_mode]
                val_rows = val_episode_rows[best_model_eval_mode]
                val_single_max_x = max((row["max_x_pos"] for row in val_rows), default=0)
                ...
                if val_summary["mean_max_x_pos"] > best_val_mean_max_x:
                    best_val_mean_max_x = val_summary["mean_max_x_pos"]
                    torch.save(agent.state_dict(), checkpoint_path / "best_max_x.pt")
                    best_max_x_summary = {
                        "global_step": global_step,
                        "criterion": "val_mean_max_x_pos",
                        "best_model_eval_mode": best_model_eval_mode,
                        "val": val_summaries,
                        "val_episodes": val_episode_rows,
                    }
                    save_json(run_path / "best_max_x_summary.json", best_max_x_summary)
                if val_single_max_x > best_val_single_max_x:
                    best_val_single_max_x = val_single_max_x
                    torch.save(agent.state_dict(), checkpoint_path / "best_single_max_x.pt")
                    best_single_max_x_summary = {
                        "global_step": global_step,
                        "criterion": "val_single_episode_max_x_pos",
                        "best_model_eval_mode": best_model_eval_mode,
                        "val_single_max_x_pos": val_single_max_x,
                        "val": val_summaries,
                        "val_episodes": val_episode_rows,
                    }
                    save_json(run_path / "best_single_max_x_summary.json", best_single_max_x_summary)
                next_eval_step += int(train_config["eval_interval"])
    ...
    final_val_summaries = {}
    final_val_episode_rows = {}
    for eval_mode in eval_modes:
        final_val_results, mode_final_val_summary = evaluate(
            agent,
            config,
            device,
            global_step,
        ...
        final_val_episode_rows[eval_mode] = final_val_results
    final_val_summary = final_val_summaries[best_model_eval_mode]
    final_val_rows = final_val_episode_rows[best_model_eval_mode]
    final_val_single_max_x = max((row["max_x_pos"] for row in final_val_rows), default=0)
    if final_val_summary["mean_total_reward"] > best_val_total_reward:
        ...
        save_json(run_path / "best_summary.json", best_summary)
    if final_val_summary["mean_max_x_pos"] > best_val_mean_max_x:
        best_val_mean_max_x = final_val_summary["mean_max_x_pos"]
        torch.save(agent.state_dict(), checkpoint_path / "best_max_x.pt")
        best_max_x_summary = {
            "global_step": global_step,
            "criterion": "val_mean_max_x_pos",
            "best_model_eval_mode": best_model_eval_mode,
            "val": final_val_summaries,
            "val_episodes": final_val_episode_rows,
        }
        save_json(run_path / "best_max_x_summary.json", best_max_x_summary)
    if final_val_single_max_x > best_val_single_max_x:
        best_val_single_max_x = final_val_single_max_x
        torch.save(agent.state_dict(), checkpoint_path / "best_single_max_x.pt")
        best_single_max_x_summary = {
            "global_step": global_step,
            "criterion": "val_single_episode_max_x_pos",
            "best_model_eval_mode": best_model_eval_mode,
            "val_single_max_x_pos": final_val_single_max_x,
            "val": final_val_summaries,
            "val_episodes": final_val_episode_rows,
        }
        save_json(run_path / "best_single_max_x_summary.json", best_single_max_x_summary)
    ...
    save_json(
        run_path / "summary.json",
        {
            ...
            "best_val_mean_max_x": best_val_mean_max_x,
            "best_val_single_max_x": best_val_single_max_x,
            "best_summary": best_summary,
            "best_max_x_summary": best_max_x_summary,
            "best_single_max_x_summary": best_single_max_x_summary,
        },
    )
    if wandb_run is not None:
